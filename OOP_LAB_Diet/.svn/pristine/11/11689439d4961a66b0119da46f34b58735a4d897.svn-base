...
public interface NutritionalElement {
	...
	default String toFileString() {
		...
		sb.append(getTypeTag()).append(":").append(getName()).append(":").append(getCalories()).append(",").append(getProteins()).append(",").append(getCarbs()).append(",").append(getFat());
		
		return sb.toString();
	}
	
	default char getTypeTag() {
		return '?';
	}
}
